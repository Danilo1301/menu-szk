...
#define MYMOD(_guid, _name, _version, _author)                                                                         \
    ...
    struct AMLInitStub                                                                                                 \
    {                                                                                                                  \
        AMLInitStub() { aml = (IAML *)GetInterface("AMLInterface"); }                                                  \
    };                                                                                                                 \
...
// Dependencies!
#define BEGIN_DEPLIST() static ModInfoDependency g_listDependencies[] = {

#define ADD_DEPENDENCY(_guid) {#_guid, ""},

#define ADD_DEPENDENCY_VER(_guid, _version) {#_guid, #_version},
...
#define ON_MOD_PRELOAD() extern "C" JNIEXPORT void OnModPreLoad()

#define ON_MOD_LOAD() extern "C" JNIEXPORT void OnModLoad()

#define ON_ALL_MODS_LOAD() extern "C" JNIEXPORT void OnAllModsLoaded()

#define ON_MOD_UNLOAD() extern "C" JNIEXPORT void OnModUnload() /*Not guaranteed*/

#define ON_GAME_CRASH()                                                                                                \
    extern "C" JNIEXPORT void OnGameCrash(                                                                             \
        const char *library, int sig, int code, uintptr_t libaddr, mcontext_t *mcontext) /*Not guaranteed*/

#define UPDATER_URL() extern "C" JNIEXPORT const char *OnUpdaterURLRequested()

#define ON_NEW_INTERFACE() extern "C" JNIEXPORT void OnInterfaceAdded(const char *name, const void *ptr)
...
inline bool strcontains(const char *str, const char *sub) { return (strstr(str, sub) != NULL); }
...
inline int clampint(const int min_val, const int max_val, int v) { return std::max(min_val, std::min(max_val, v)); }
inline void clampint(const int min, const int max, int *v)
{
    if (v)
        *v = clampint(min, max, *v);
}
...
inline bool randbool() { return ((rand() & 1) == 0); }
...
inline float randfloat(float min, float max) { return min + ((float)rand() / (float)RAND_MAX) * (max - min); }
inline int randsign() { return (rand() & 1) ? 1 : -1; }
...
inline float fastsin(float x) // much better than above but not guaranteed to be the best!
{
    ...
    float res = x_reduced * (1.0f + x2 * (-0.1666665671f + x2 * (0.0083321510f + x2 * (-0.0001951529f))));
    return res * sign;
}
...
template <typename T> int getsign(T val) { return (T(0) < val) - (val < T(0)); }
...
inline bool evenvalue(int v) { return ((v & 0x1) == 0); }
inline bool oddvalue(int v) { return !evenvalue(v); }
...
class ModInfo
{
  public:
    ...
    {
        /* No buffer overflow! */
        strxcpy(this->szGUID, szGUID, sizeof(ModInfo::szGUID));
        this->szGUID[sizeof(ModInfo::szGUID) - 1] = '\0';
        strxcpy(this->szName, szName, sizeof(ModInfo::szName));
        this->szName[sizeof(ModInfo::szName) - 1] = '\0';
        strxcpy(this->szVersion, szVersion, sizeof(ModInfo::szVersion));
        this->szVersion[sizeof(ModInfo::szVersion) - 1] = '\0';
        strxcpy(this->szAuthor, szAuthor, sizeof(ModInfo::szAuthor));
        ...
        if (sscanf(this->szVersion, "%hu.%hu.%hu.%hu", &version.major, &version.minor, &version.revision,
                &version.build) < 4)
        {
